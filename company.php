...
require_once "app/Utilities/DomainUtils.php";
$set = setting($PDO);
...
$row_id = $row['id'];
$total = $PDO->query("SELECT `id`  FROM `review` WHERE `id_com`= $row_id and `moderation` = 1 and `text` != '' $post")
    ->rowCount();
...
<head>
	...
	<meta name="description" content='Актуальные отзывы о компании «<?= $row['name'] ?>»<?php $dp = $page + 1;
    if
    ($page + 1 != 1)
    {
        echo " — страница " . $dp;
    } ?>. Почитайте отзывы, напишите свой или оставьте заявку на ремонт
    квартиры на сайте RATING REMONT.'>
    <?php
    //Линки next и prev
    if ($page + 1 == 1)
    {
        echo '<link rel="next" href="https://' . $_SERVER['HTTP_HOST'] . '/otzyvy-' . $name . '/2' . $postfix .
            '" />';                    //следующая
    } else if ($page + 1 == $pages - 1)
    {
        echo '<link rel="prev" href="https://' . $_SERVER['HTTP_HOST'] . '/otzyvy-' . $name . '/' . ($page + 1) .
            $postfix . '"/>';  //Назад
    } else
    {
        echo '<link rel="next" href="https://' . $_SERVER['HTTP_HOST'] . '/otzyvy-' . $name . '/' . ($page + 2) .
            $postfix . '" />'; //следующая
        echo '<link rel="prev" href="https://' . $_SERVER['HTTP_HOST'] . '/otzyvy-' . $name . '/' . ($page) . $postfix .
            '"/>';             //Назад
    }
	...
	<meta property="og:description"
	      content="Актуальные отзывы о компании «<?= $row['name'] ?>». Почитайте отзывы, напишите свой или оставьте заявку на ремонт квартиры на сайте RATING REMONT." />
		...
		<div class="content__item-left">
			<div class="snippet company">
				<div class="company__title">
					<h1>Отзывы о компании <?= $row['name'] ?><span>Дата основания: <?= date('d.m.Y',
                                $row['data']) ?></span></h1>
				</div>
				<div class="compaty__item">
					...
					<div class="company__attributes">
						...
						<?php if(\app\Utilities\DomainUtils::isMainDomain()) { ?>
						<div>
							<span>Адрес: </span>
			...
			<div class="button-panel">
				...
				<button class="company__bottom-green request_modal" data-id="<?= $row['id'] ?>"
				        data-key="<?= $func->hash($row_id) ?>" data-type="request" data-val="rating">Оставить заявку
				</button>
			</div>
			<!--review-->
            <?php
            ...
            if ($countReview > 0)
            {
					...
					<div class="snippet review">
						<div class="review__header">
							...
							<div class="review__like <?php echo $commentRow['rev'] == 1 ? 'positive' : 'negativ'; ?>">
								<i></i>
                                <?php echo $commentRow['rev'] == 1 ? 'Рекомендую' : 'Не рекомендую'; ?>
							</div>
						</div>
						<div class="review__text">
                            <?= $commentRow['text']; ?>
                            <?php if ($commentRow['service'] != '0') { ?>
								<div><b>Договор:</b>
									=$commentRow['service']?>
								</div><?php } ?>
						...
						<div class="review__footer">
							...
							<button type="button" class="company__bottom-green comment_modal"
							        ...
							        data-type="comment" data-val="rating">КОММЕНТИРОВАТЬ
							</button>
                        ...
                        <?php
                        $comm =
                            $PDO->query("SELECT * FROM `comment` WHERE `review` = $reviewID and `moderation` = 1 and `type` = 'rating'");
                        if ($comm->rowCount() > 0)
                        {
                            ?>
							<div class="comments">
				...
				<div class="page_nav">
                    ...
                    if ($page >= 1)
                    {
                        echo '<a href="/otzyvy-' . $name . '/1' . $postfix .
                            '#rew_block" class="oneLink"></a>';  //На первую
                        echo '<a href="/otzyvy-' . $name . '/' . $page . $postfix .
                    ...
                    for ($j = 1; $j < $pages; $j++)
                    {
                        if ($j >= $start && $j <= $end)
                        {
                            ...
                            if ($j == ($page + 1))
                                echo '<a href="/otzyvy-' . $name . '/' . $j . $postfix . '#rew_block" class="active">' .
                                    $j . '</a>';
                            else
                    ...
                    if ($j > $page && ($page + 2) < $j)
                    {
                        echo '<a href="/otzyvy-' . $name . '/' . ($page + 2) . $postfix .
                            '#rew_block" class="nav-next"></a>';
                        echo '<a href="/otzyvy-' . $name . '/' . ($j - 1) . $postfix .
